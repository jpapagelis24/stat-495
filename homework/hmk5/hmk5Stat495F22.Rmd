...
# Introduction 
We are interested in determining the features that are predictive of homes that are selling for more than half a million dollars. We are only interested in whether the price is over or under \$500,000, so will transform the price into a binary response variable where the the price is either over \$500,000 or it isn't. Then because the response variable is binary, we will use logistic regression to create the best model that we can. We will also perform a non-parametric analysis using classification trees in order to help visualize what predictors are best indicators of a house selling for greater than \$500,000. Finally, we will be able to compare the models to help answer the real estate developer's questions.
...
```{r, include = TRUE, fig.width=9, fig.height=4, warning=FALSE}
grid.arrange(fig1, fig2, ncol = 2)
```

Additionally, from the Scatterplot of Bedrooms and Price below, there appears to be an outlier so we will remove that observation.
...
From the scatterplot below of the latitude and the longitude, it seems like the houses are all bunched together near each other (close to Seattle) so the results of this analysis can only be generalized to that region of the state.
...
Next, since we want to know if the price of the house is greater or less than \$500,000, we will transform the price variable so that it is an binary predictor that is `1` if the price of the house is greater than \$500,000 and `0` if the price of the house is less than \$500,000. It appears that there are 12,546 houses with price below \$500,000 and 9,050 houses with a price above \$500,000.
...
The first models that we are going to be fitting are GLMs (or Generalized Linear Models) which are extensions of ordinary linear regression (fitting a least-squares curve) to cases where the response variables are in an exponential family form. This can be response variables that are binomial, Poisson, gamma, or beta. Since we transformed `price` into a binary variable, it can now be modeled as a binomial distribution. The model that we use for a binary response variable is a logistic regession model.

First, we are going to fit the logistic model using all of the variables that we have not removed from the data set. We use the training set to create the model. 
