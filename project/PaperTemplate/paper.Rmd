...
# Introduction
Bootstrapping is an important tool in Statistics, even more so, now that there is access to a higher-level of computing power. We use bootstrapping to find an estimate of a desired population parameter from a given sample, without making any assumptions about any underlying distributions of the sample. 
...
run_simulation_gamma <- function(num_reps, sample_size){
  ...
  for (i in 1:simulation_reps){
    ...
    ci[i, 1] <- trueparameter >= output$`Standard Interval`[1] & 
      trueparameter <= output$`Standard Interval`[2]
    ci[i, 2] <- trueparameter >= output$`Percentile Interval`[1] & 
      trueparameter <= output$`Percentile Interval`[2]
    ci[i, 3] <- trueparameter >= output$`BC Interval`[1] &
      trueparameter <= output$`BC Interval`[2]
    ci[i, 4] <- trueparameter >= output$`BCa Interval`[1] & 
      trueparameter <= output$`BCa Interval`[2]
    ci[i, 5] <- trueparameter >= output$`Studentized Interval`[1] & 
      trueparameter <= output$`Studentized Interval`[2]
  }
...
```{r}
run_simulation_log_normal <- function(num_reps, sample_size){
  set.seed(495)
  simulation_reps <- num_reps
...
```{r}
run_simulation_normal <- function(num_reps, sample_size){
  set.seed(495)
  simulation_reps <- num_reps
  trueparameter <- 0
  n <- sample_size
...
collect_data_gamma <- function(sim_reps) {
  ...
  simulation_reps <- sim_reps
  sample_sizes <- c(10, 25, 50, 100, 250)
  
  for (i in 1:5) {
    output2 <- run_simulation_gamma(simulation_reps, sample_sizes[i])
    data2[i, 1] <- sample_sizes[i]
    data2[i, 2:6] <- output2
  }
...
collect_data_log_normal <- function(sim_reps) {
  ...
  for (i in 1:5) {
    output2 <- run_simulation_log_normal(simulation_reps, sample_sizes[i])
    data2[i, 1] <- sample_sizes[i]
    data2[i, 2:6] <- output2
  }
...
collect_data_normal <- function(sim_reps) {
  ...
  for (i in 1:5) {
    output2 <- run_simulation_normal(simulation_reps, sample_sizes[i])
    data2[i, 1] <- sample_sizes[i]
    data2[i, 2:6] <- output2
  }
...
```{r, cache = TRUE}
set.seed(495)
gamma_data <- collect_data_gamma(30)
normal_data <- collect_data_normal(30)
log_normal_data <- collect_data_log_normal(30)
```

```{r}
  gamma_data1 <- gamma_data %>%
    pivot_longer(cols = 2:6, names_to = "method", values_to = "prop") 
  
  gamma_fig <- ggplot(data = gamma_data1, aes(x = Sample_Size, y = prop, color = method))+
    geom_point() +
    geom_line() + 
    labs(title = "Proportion of CIs that Contain the True Parameter from Gamma Distibution",
         x = "Sample Size", y = "Proportion (of xyz Simulations)", color = "Method") +
    theme_light()
  
  gamma_table <- gamma_data %>%
    kable(booktabs = TRUE, align = "c", caption = "gamma", col.names = gsub("[_]", " ", names(gamma_data)))
...
#simulation_reps <- 30
#simulation_rep <- seq(1,simulation_reps)
#sample_size <- 20
#  simulation_reps <- num_reps

run_simulation_normal1 <- function(simulation_rep){
  trueparameter <- 0
  ...
  collect_data_normal1 <- function(sim_reps) {
  data2 <- matrix(nrow = 5, ncol = 6)
  ...
  for (i in 1:5) {
    #output2 <- run_simulation_normal1(simulation_reps)
    simulation_rep <- seq(1,sim_reps)
    results <- mclapply(simulation_rep, run_simulation_normal1, mc.cores = detectCores())
