...
```{r run simulation log normal}

#Third, we sample from a Log-Normal distribution with mean log of 0 and a mean standard deviation of 1. Therefore, the true population mean of this distribution is $e^{\frac{1}{2}}$ [@Guillaume21].
...
collect_data_log_normal <- function(sim_reps) {
  ...
  for (i in 1:7) {
    simulation_rep <- seq(1,sim_reps)
    results <- mclapply(simulation_rep, run_simulation_log_normal, 
                        mc.cores = detectCores())
    df <- data.frame(matrix(unlist(results), nrow=length(results), byrow=TRUE))
...
gamma_data_4_128 <- collect_data_gamma(sim_reps)
write.csv(gamma_data_4_128,"gamma_data_4_128.csv", row.names = FALSE)

normal_data_4_128 <- collect_data_normal(sim_reps)
write.csv(normal_data_4_128,"normal_data_4_128.csv", row.names = FALSE)

log_normal_data_4_128 <- collect_data_log_normal(sim_reps)
write.csv(log_normal_data_4_128,"log_normal_data_4_128.csv", row.names = FALSE)

beta_data_4_128 <- collect_data_beta(sim_reps)
write.csv(beta_data_4_128,"beta_data_4_128.csv", row.names = FALSE)
```
...
lognormal_df_long <- lognormal_df %>%
  pivot_longer(cols = 2:6, names_to = "method", values_to = "prop") 
    
lognormal_fig <- ggplot(data = lognormal_df_long, 
...
lognormal_table <- lognormal_df %>%
  kable(booktabs = TRUE, align = "c", caption = "log normal", 
        col.names = gsub("[_]", " ", names(lognormal_df)))

lognormal_df <- read.csv(file = 'log_normal1_data_10000.csv')
...
gamma_df <- read.csv(file = 'gamma_data1_10000.csv')
normal_df <- read.csv(file = 'normal_data1_10000.csv')
log_normal_df <- read.csv(file = 'log_normal1_data_10000.csv')

gamma_df_long <- log_normal_df %>%
  pivot_longer(cols = 2:6, names_to = "method", values_to = "prop") 
...
gamma_table <- gamma_df %>%
  kable(booktabs = TRUE, align = "c", caption = "Sampled From Gamma Dist.", 
        col.names = gsub("[_]", " ", names(gamma_df))) 
...
for(i in 1:num_sample_sizes) {
  ...
  for (j in 1:sim_reps) {
    ...
    results <- boot(data = values, statistic = mean.fun, R = boot_reps)
    results
    #plot(results)
    ci <- boot.ci(results, type = "all")
    
    norm_contains[j] <- true_mean >= ci$normal[2] & true_mean <= ci$normal[3]
    basic_contains[j] <- true_mean >= ci$basic[4] & true_mean <= ci$basic[5]
    student_contains[j] <- true_mean >= ci$basic[4] & true_mean <= ci$basic[5]
    percentile_contains[j] <- true_mean >= ci$basic[4] & true_mean <= ci$basic[5]
    bca_contains[j] <- true_mean >= ci$basic[4] & true_mean <= ci$basic[5]
    
  }
  
  master[i, 1] <- sample_sizes[i]
  master[i, 2] <- sum(norm_contains)/sim_reps
  master[i, 3] <- sum(basic_contains)/sim_reps
  master[i, 4] <- sum(student_contains)/sim_reps
  master[i, 5] <- sum(percentile_contains)/sim_reps
  master[i, 6] <- sum(bca_contains)/sim_reps
