...
```{r}
findCIs <- function(sampleinput, trueparameter, nboot) {
  
  sample_mean <- mean(~ Dec, data = sampleinput)
  sample_sd <- sd(~ Dec, data = sampleinput)
  ...
  # find BCa interval
  theta <- function(x){mean(x)}
  jackknife_results <- bootstrap::jackknife(sampleinput$Dec, theta)
  ...
  # find studentized
  q <- unname(quantile(boot_t_stat$t_stat, c(0.025, 0.975)))
  lower <- q[1]
  upper <- q[2]
  t_interval <- c(orig_mean - upper*orig_se, orig_mean - lower*orig_se)

  output <- c(standard_interval, percentile_interval, bc_interval, bca_interval, t_interval)
