...
```{r, eval = FALSE}
    ...
    gamma_fig <- ggplot(data = gamma_data1, aes(x = Sample_Size, y = prop, 
      ...
      labs(title = "Proportion of CIs that Contain the True Parameter 
           from Gamma Distibution", x = "Sample Size", 
           y = "Proportion (of xyz Simulations)", color = "Method") +
      theme_light()
    
    gamma_table <- gamma_data %>%
      kable(booktabs = TRUE, align = "c", caption = "gamma", 
            col.names = gsub("[_]", " ", names(gamma_data)))
